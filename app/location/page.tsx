...
export default function LocationPage() {
  ...
  if (!warehouseId && !loading) return <div className="min-h-screen bg-[#2A1B12] flex items-center justify-center text-[#A15C48]">No Warehouse Selected</div>;
    ...
    <div className="min-h-screen relative font-sans flex items-center justify-center p-4 sm:p-8 bg-[#2A1B12]"> {/* Lighter Background Outside */}
        ...
        <div className="relative z-20 flex flex-col h-full">
            ...
            <div className="flex-1 overflow-y-auto p-6">
                {loading ? (
                    ...
                    <div className="text-center py-12 border border-[#A15C48]/10 border-dashed rounded-xl bg-[#2A1B12]/40"><p className="text-[#F7D6CF]/50">No locations found in this warehouse.</p></div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {locations.map((loc) => {
                            const inventory = MOCK_INVENTORY[loc.id] || [];
                            return (
                                <div key={loc.id} className="group bg-[#2A1B12]/60 hover:bg-[#2A1B12]/80 border border-[#A15C48]/20 hover:border-[#A15C48]/40 rounded-2xl p-5 transition-all backdrop-blur-sm">
                                    {/* Location Header */}
                                    ...
                                    {/* Inventory Section */}
                                    <div className="bg-[#2A1B12]/40 rounded-xl p-3 border border-[#A15C48]/10">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="w-full max-w-md bg-[#2A1B12] border border-[#A15C48]/30 rounded-3xl shadow-2xl p-8 animate-in zoom-in-95 relative overflow-hidden">
                ...
                <div className="relative z-10">
                    ...
                    <form onSubmit={handleLocationSubmit} className="space-y-4">
                        <div>
                            ...
                            <input required value={locForm.name} onChange={e => setLocForm({...locForm, name: e.target.value})} className="w-full mt-1 bg-[#2A1B12]/80 border border-[#A15C48]/30 rounded-xl px-4 py-3 text-white focus:border-[#A15C48] outline-none placeholder-[#F7D6CF]/30" placeholder="e.g. Shelf A"/>
                        </div>
                        <div>
                            <label className="text-xs font-bold text-[#A15C48] uppercase tracking-wider">Short Code</label>
                            <input required value={locForm.shortCode} onChange={e => setLocForm({...locForm, shortCode: e.target.value})} className="w-full mt-1 bg-[#2A1B12]/80 border border-[#A15C48]/30 rounded-xl px-4 py-3 text-white focus:border-[#A15C48] outline-none placeholder-[#F7D6CF]/30" placeholder="e.g. SH-A"/>
                        </div>
