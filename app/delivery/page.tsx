...
const DeliveryList = ({ onSelectOrder, onCreateNew }: DeliveryListProps) => {
  const [deliveries, setDeliveries] = useState<typeof MOCK_DELIVERIES>(MOCK_DELIVERIES);
...
const DeliveryDetail = ({ order, onBack, onStatusChange }: DeliveryDetailProps) => {
  const [currentStatus, setCurrentStatus] = useState<Status>(order.status as Status);
  ...
  const statusPath: Status[] = ['Draft', 'Waiting', 'Ready', 'Done'];
  const currentStepIndex = statusPath.indexOf(currentStatus);
