...
const INITIAL_WAREHOUSES: Warehouse[] = [
    { id: '1', name: 'Central Hub', shortCode: 'WH-NY', address: '123 Broadway, New York, NY', description: 'Main distribution center' },
    { id: '2', name: 'West Coast Depot', shortCode: 'WH-LA', address: '456 Sunset Blvd, Los Angeles, CA', description: 'Imports form APAC' },
];
...
export default function WarehousePage() {
    ...
    const [whForm, setWhForm] = useState({ name: '', shortCode: '', address: '', description: '' });
    ...
    const handleWarehouseSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingItem) {
            setWarehouses(prev => prev.map(w => w.id === editingItem.id ? { ...w, ...whForm } : w));
            showToast('Warehouse updated', 'success');
        } else {
            setWarehouses(prev => [...prev, { id: Math.random().toString(), ...whForm }]);
